...
% Forcing
if dolinear==1
term=HH*DDA*eta; % linear: eta is anomalies ! 
else
term=(HH*DDA*eta-so); % nonlinear
end
if dofreewaves==1; term=term*0; end
...
Ke=longwavefd(K,ekr,1,term,dx,dt);
end
% Rossbys 
for im=1:nrm
...
Rme(:,im)=longwavefd(Rm(:,im),ekr,-1/(2*im+1),term,dx,dt);
end
end% loop im
...
if dolinear==1
% deterministic linear (around RCE state u,v,q,o=0 and Ha=so, with so at current time)
Ze=Z+dt*(QQ-1)*HH*DDA*eta; % linear: eta is anomalies ! 
etae=eta+dt*GG/HH*so.*(Ze-QQ*oe);
else
% deterministic nonlinear
Ze=Z+dt*((QQ-1)*HH*DDA*eta+sq-QQ*so);% deterministic (further splits Z-a)
etae=eta.*exp(GG*(Ze-QQ*oe)*dt);% split method requires oe
end
end% dostocha
%
