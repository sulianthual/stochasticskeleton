...
nyk=             5        ;%X% % Meridional Truncation (strips/Hermites)
nrm=             3        ;%X% % Rossby solved (can have nrm>=nyk-2,for which Sm=psim=0)
%
% Time Grid
...
backgroundtype=  1        ; % type of background to use, 0=homogeneous, 1=gaussian warmpool
...
doseaso=         1        ; %X% % do a seasonal cycle (where the WP displaces from +-yo each year)
pseaso=          365       ; % seasonal cycle period (in days)
yseaso=          0.6       ; %X% % maximal yoo of seasonal cycle (otherwise yoo is used, e.g. for initiation/diagnostics)
%
% Files
